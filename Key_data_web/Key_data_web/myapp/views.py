...
def random_select(request):
    ...
    rep_dict = user_rep_map[request.session['user_id']]['rep_dict']
    format_cor_ans = user_rep_map[request.session['user_id']]['format_cor_ans']
    ...
    while True:
        ...
        if rep_dict[next_web]:
            ...
            if next_web.split('_')[1] != 'rep':
                ...
                attack_web_id = wid
                user_rep_map[request.session['user_id']]['attack_user_id'] = attack_user_id
                user_rep_map[request.session['user_id']]['attack_ans'] = attack_ans
                user_rep_map[request.session['user_id']]['attack_web_id'] = attack_web_id
                if attack_ans:
...
def init_all(request):
    ...
    user_rep_map[request.session['user_id']] = {}
    user_rep_map[request.session['user_id']]['rep_time'] = rep_time
    user_rep_map[request.session['user_id']]['rep_time'] = rep_time
    user_rep_map[request.session['user_id']]['w1_ans'] = {}
    user_rep_map[request.session['user_id']]['w2_ans'] = {}
    user_rep_map[request.session['user_id']]['w3_ans'] = {}
    user_rep_map[request.session['user_id']]['attack_ans'] = None
    user_rep_map[request.session['user_id']]['attack_web_id'] = None
    user_rep_map[request.session['user_id']]['attack_user_id'] = None
    user_rep_map[request.session['user_id']]['res'] = []
    rep_dict = {'w1_rep':rep_time-1, 'w2_rep':rep_time-1, 'w3_rep':rep_time-1,}

    all_name_map = {1:d1, 2:d2, 3:d3}  # 将文件数据映射到相应变量
    user_rep_map[request.session['user_id']]['format_cor_ans'] = format_cor_ans
    user_rep_map[request.session['user_id']]['all_name_map'] = all_name_map
    m = 1
    n = 1
    # 设置rep_dict字典以追踪重复操作
    ...
    # if True:
        for i in range(1, user_id):
            ...
            if now_attack_n == n:
                break
    user_rep_map[request.session['user_id']]['rep_dict'] = rep_dict
...
def get_start_w1(request):
    ...
    if request.method == 'POST':
        user_id = request.session['user_id']
        w1_ans = user_rep_map[request.session['user_id']]['w1_ans']

        all_keys = []
        for i in d1:
            all_keys.append(i)
        for key in all_keys:
            if request.POST.get(key).replace(' ','') == '1':
        ...
        for event in data:
            ...
            res = user_rep_map[request.session['user_id']]['res']
            res.append([user_id, 1, False, '', event['key'] + ' ' + event['type'], event['time']])
            user_rep_map[request.session['user_id']]['res'] = res
        # 跳转到第二个网页
...
def get_start_w2(request):
    ...
    if request.method == 'POST':
        w2_ans = user_rep_map[request.session['user_id']]['w2_ans']
        user_id = request.session['user_id']
        all_keys = []
        for i in d2:
            all_keys.append(i)
...
def get_start_w3(request):
    # global count, w1_ans, w2_ans, w3_ans, user_id, res
    if request.method == 'POST':
        w3_ans = user_rep_map[request.session['user_id']]['w3_ans']
        user_id = request.session['user_id']
        all_keys = []
        for i in d3:
            all_keys.append(i)
...
def w_attack(request):
    # global count, w1_ans, w2_ans, w3_ans, user_id, res
    attack_ans = user_rep_map[request.session['user_id']]['attack_ans']
    attack_web_id = user_rep_map[request.session['user_id']]['attack_web_id']
    attack_user_id = user_rep_map[request.session['user_id']]['attack_user_id']
    user_id = request.session['user_id']
    if request.method == 'POST':
        now_ans = {}
        all_keys = []
...
def finished_web(request):
    # for key, value in user_rep_map[request.session['user_id']].items():
    #     print(key, value)
    # 当所有输入都完成后，将数据写入数据库
    db_file = 'key_data.db'
    conn = sqlite3.connect(db_file)
    ...
    w3_ans = user_rep_map[request.session['user_id']]['w3_ans']
    user_id = request.session['user_id']
    all_name_map = user_rep_map[request.session['user_id']]['all_name_map']
